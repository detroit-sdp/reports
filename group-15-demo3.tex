...
The other key parameters are mostly self-explanatory other than the following. [min/max]_trans_vel is the speed of the robot when moving in a straight line. path_distance_bias determines how closely the robot need follow the path it works out. Given our robot always calculated an accurate path, we increased this value. goal_distance_bias smoothes the trajectory of the path and makes it more efficient. Again, we decided to increase this. Finally, occdist_scale determines how 'afraid' the robot is of obstacles. We lowered this to make it behave a little safer. All our parameter changes are detailed in table~\ref{tab:params}.
